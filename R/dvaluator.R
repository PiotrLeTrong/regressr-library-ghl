...
dvaluator <- function(depvar, order = F, panel = F) {
  ...
  writeLines(paste("\nNow that you know the nature of you dependent variable and what model you'd like to run, I recommend using the next function in the library, optimizer. All you have to do is feed it a dependent variable and a set of independent variables. A dependent variable is the outcome variable you want to know something about. The independent variables are those factors you think explain your dependent variable. For instance, if I'm interested in test scores, the scores would be my dependent variable, and my independent variables might be data on the test takers and the schools they attend."))
}